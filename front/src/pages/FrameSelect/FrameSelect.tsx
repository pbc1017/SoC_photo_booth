...
export const FrameSelect = (): JSX.Element => {
  const location = useLocation();
  const { numPhoto, compressedImages } = location.state;
  ...
  const handleNextClick = () => {
    ...
    console.log(compressedImages);
    navigate("/filterselect", { state: { 
        numPhoto : numPhoto,
        compressedImages : compressedImages,
        selectedFrame : selectedOption
    } }); // selectedOption을 state로 전달
  };

  const handlePrevClick = () => {
    navigate("/photoselect", {state:{numPhoto : numPhoto}}); // 이전 버튼 클릭시 "/" 라우터로 이동
    };
  ...
  return (
    <div>
      <PhotoFrameGrid numPhoto = {numPhoto} photoImages = {compressedImages} selectedFrame = {selectedOption} />
        <div>
